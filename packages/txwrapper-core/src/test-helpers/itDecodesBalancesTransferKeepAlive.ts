...
/**
 * Information common to all decoded balances keep alive transfers used for testing.
 *
 * @param decoded info from a decoded tx
 */
export function itDecodesBalancesTransferKeepAlive(
	decoded: DecodedSignedTx | DecodedSigningPayload | DecodedUnsignedTx,
): void {
	expect(decoded.method.pallet).toBe('balances');
	expect(decoded.method.name).toBe('transferKeepAlive');
	expect(decoded.method.args.dest).toEqual(
		TEST_METHOD_ARGS.balances.transferKeepAlive.dest,
	);
